...
namespace model {
    ...
    QVariant Leaderboard::data(const QModelIndex &index, int role) const {
        ...
        switch (role) {
            ...
            case Roles::Date:
                return scoreLine.date();
            default:
                return {};
        }
    }

    void Leaderboard::connectEntry(model::ScoreLine *const entry) {
        connect(entry, &model::ScoreLine::scorelineChanged,
                this, [this]() { onEntryPropertyChanged({Roles::PlayerId, Roles::Score, Roles::Date}); });
    }


    void Leaderboard::onEntryPropertyChanged(const QVector<int> &roles) {
